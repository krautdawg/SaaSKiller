...
import { getStrings, t as translate } from './i18nService.js';
...
export async function generateAuditPDF(auditData) {
  ...
  // Helper for translating with args
  const trans = (key, ...args) => translate(t, key, ...args);
    ...
    try {
      ...
      if (auditData.tierName) {
        doc
          .fontSize(12)
          .fillColor('#6B7280')
          .text(`${trans('bleed.subscriptionPlan')}: ${auditData.tierName}`)
          .moveDown(0.2);
      }
      ...
      doc
        .fontSize(12)
        .fillColor('#6B7280')
        .text(`${trans('audit.teamSize')}: ${auditData.teamSize} ${trans('audit.users')}`)
        .moveDown(1.5);
      ...
      doc
        .fontSize(16)
        .fillColor('#DC2626')
        .text(trans('bleed.title'), { align: 'center' })
        .moveDown(0.5);
      ...
      doc
        .fontSize(10)
        .fillColor('#9CA3AF')
        .text(trans('bleed.note'), { align: 'center' })
        .moveDown(1.5);
      ...
      doc
        .fontSize(14)
        .fillColor('#111827')
        .text(trans('bleed.build.title'), { align: 'left' })
        .moveDown(0.8);

      doc
        .fontSize(11)
        .fillColor('#6B7280')
        .text(`${trans('bleed.build.cost')}: $${auditData.buildCostMin.toLocaleString()} - $${auditData.buildCostMax.toLocaleString()}`)
        .moveDown(0.5);

      doc
        .fontSize(11)
        .fillColor('#059669') // green-600
        .text(`${trans('bleed.build.savings')}: $${auditData.savingsAmount.toLocaleString()}`)
        .moveDown(0.5);
      ...
      if (auditData.roiMonths) {
        doc
          .fontSize(11)
          .fillColor('#6B7280')
          .text(`${trans('bleed.build.roi')}: ${auditData.roiMonths} ${trans('bleed.build.months')}`)
          .moveDown(1.5);
      } else {
        doc.moveDown(1);
      }
      ...
      if (auditData.featuresKept && auditData.featuresKept.length > 0) {
        doc
          .fontSize(14)
          .fillColor('#111827')
          .text(trans('quote.features.kept'), { align: 'left' })
          .moveDown(0.5);
      ...
      if (auditData.featuresRemoved && auditData.featuresRemoved.length > 0) {
        doc
          .fontSize(14)
          .fillColor('#111827')
          .text(trans('quote.features.removed'), { align: 'left' })
          .moveDown(0.5);
      ...
      if (auditData.customFeatures && auditData.customFeatures.length > 0) {
        doc
          .fontSize(14)
          .fillColor('#111827')
          .text(trans('quote.features.custom'), { align: 'left' })
          .moveDown(0.5);
      ...
      doc
        .fontSize(9)
        .fillColor('#9CA3AF')
        .text(`${trans('footer.tagline')} - ${trans('hero.title.line1')} ${trans('hero.title.line2')}`, {
          align: 'center'
        })
        .moveDown(0.3);
      ...
      doc
        .fontSize(8)
        .fillColor('#D1D5DB')
        .text(new Date().toLocaleString(lang === 'de' ? 'de-DE' : 'en-US'), { align: 'center' });
